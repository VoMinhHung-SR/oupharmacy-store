    ...
    <div className="space-y-6 py-8 px-4">
      ...
      <div className="rounded-lg border bg-white p-6 md:p-8">
        ...
        <div className="border-t pt-6 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((item: { medicine_unit_id?: number; quantity?: number; price?: number; subtotal?: number }, idx: number) => {
                  const qty = item.quantity ?? 0
                  const price = item.price ?? 0
                  const subtotal = item.subtotal ?? qty * price
                  return (
                    <tr key={item.medicine_unit_id ?? idx} className="border-b">
                      <td className="py-2 pr-4">Sản phẩm #{item.medicine_unit_id}</td>
        ...
        <div className="flex flex-wrap gap-4 mt-8 pt-6 border-t">
          ...
          <Link
            href="/"
            className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          >
            Tiếp tục mua sắm
          </Link>
        </div>
