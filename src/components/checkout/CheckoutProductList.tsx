...
export function CheckoutProductList({ items, lineSubtotal, hideProductNames = false }: CheckoutProductListProps) {
  ...
  const hasAnyCatalogPromo = items.some((item) =>
    cartLineHasDirectPromo(item.price, item.listPrice),
  )
              ...
              <div className="flex items-start gap-2.5 sm:gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <CartLineDirectPromoBar
                    ...
                    listPriceSnapshot={item.listPrice}
                    promoEndsAt={catalogPromoEndsAt}
                    className="mt-1.5"
                  />
                </div>
                <CartLinePriceDisplay
                  saleAmount={item.price * item.qty}
                  listAmount={
                    item.listPrice != null && item.listPrice > item.price
                      ? item.listPrice * item.qty
                      : null
                  }
                  size="sm"
                  className="shrink-0 self-start text-right"
                />
              </div>
            </li>
          ))}
