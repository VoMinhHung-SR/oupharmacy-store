...
export function MedicineSearchModal({
  ...
  const toggleProduct = (productId: number, productName: string, packing?: string, imageUrl?: string) => {
    setDraft((prev) => {
    ...
    <OfferSheet
      ...
      footer={
        <div className="border-t border-slate-100 px-5 py-4">
          <Button className="w-full" onClick={() => onComplete(draft)}>
            Hoàn tất
          </Button>
        </div>
      }
              ...
              <li key={product.id}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {packing ? <span className="block truncate text-sm text-gray-500">{packing}</span> : null}
                  </span>
                  <span
