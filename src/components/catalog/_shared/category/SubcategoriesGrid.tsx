...
export function SubcategoriesGrid({
  ...
  // Memoize subcategory name extraction
  const extractedSubcategories = useMemo(() =>
    subcategories.map(subcat => ({
      ...subcat,
      displayName: subcat.name.split(' > ').pop() || subcat.name
    })),
    [subcategories]
  )
  ...
  return (
    <div className="mb-8">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        {extractedSubcategories.map((subcat) => (
          <Link
            key={subcat.slug}
            href={`/${subcat.slug}`}
          ...
          >
            ...
            <div className="w-12 h-12 mb-3 flex items-center justify-center bg-gray-50 rounded-lg group-hover:bg-primary-50 transition-colors">
              <CategoryIcon 
                ...
                className="w-8 h-8 text-gray-600 group-hover:text-primary-600 transition-colors"
                aria-hidden="true"
              />
            </div>
